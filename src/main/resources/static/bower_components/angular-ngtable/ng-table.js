...
!function(a,b){
    ...
    b.factory("ngTableParams",["$q",function(b){
        var c=function(a){
            ...
            this.parameters=function(b,d){
                ...
                var a=[];
                for(var b in g.sorting)a.push(("asc"===g.sorting[b]?"+":"-")+b);return a
                ...
                e.promise.then(function(b){
                    ...
                    for(var f in b){
                        var g=b[f],h=a.isFunction(d)?d(g):g[d];
                        e[h]=e[h]||{
                            data:[]
                        },e[h].value=h,e[h].data.push(g)
                    }
                    var i=[];
                    for(var j in e)i.push(e[j]);c.resolve(i)
                }),this.getData(e,f)
                ...
                for(key in g)if(g.hasOwnProperty(key)){
                    var d=g[key],e=encodeURIComponent(key);
                    if("object"==typeof d){
    ...
    return b.directive("ngTable",["$compile","$q","$parse",function(b,d,e){
            ...
            compile:function(c){
                ...
                return a.forEach(a.element(c[0].querySelector("tr:not(.ng-table-group)")).find("td"),function(b){
                    ...
                    if(!c.attr("ignore-cell")||"true"!==c.attr("ignore-cell")){
                        ...
                        },i=c.attr("filter")?e(c.attr("filter"))():!1,j=!1;
                    ...
                    if(c.$loading=!1,c.$columns=d,c.$watch(g.ngTable,function(b){
                        ...
                        var h=a.element(document.createElement("thead")).attr("ng-include","templates.header"),i=a.element(document.createElement("div")).attr("ng-include","templates.pagination");
        ...
        a.put("ng-table/filters/select.html",'<select ng-options="data.id as data.title for data in column.data" ng-model="params.filter()[name]" ng-show="filter==\'select\'" class="filter filter-select form-control"> </select>'),
        ...
        a.put("ng-table/header.html",'<tr style=" border-color: inherit;" class="table table-striped text-left"> \n\
    <th ng-repeat="column in $columns" \n\
           ng-click="sortBy(column)" ng-show="column.show(this)" \n\
            ng-init="template=column.headerTemplateURL(this)" class="header" > \n\
        <div ng-if="!template" class="ng-table-header ng-scope" ng-bind="parse(column.title)"></div>\n\
        <div ng-if="template">\n\
            <div ng-include="template"></div>\n\
        </div> \n\
    </th> \n\
</tr> <tr ng-show="show_filter" class="ng-table-filters"> <th ng-repeat="column in $columns" ng-show="column.show(this)" data-title-text="{{column.title}}" class="filter"> <form ng-submit="doFilter()"> <input type="submit" tabindex="-1" style="position: absolute; left: -9999px; width: 1px; height: 1px;"/> <div ng-repeat="(name, filter) in column.filter"> <div ng-if="column.filterTemplateURL"> <div ng-include="column.filterTemplateURL"></div> </div> <div ng-if="!column.filterTemplateURL"> <div ng-include="\'ng-table/filters/\' + filter + \'.html\'"></div> </div> </div> </form> </th> </tr>'),
       
        a.put("ng-table/pager.html",'\
   <footer  class="panel-footer">\n\
<div class="row" ng-init="pageCount=50">\n\
  <div ng-hide="pages.length>0?false:true" class="col-sm-4 hidden-xs"> \n\
   <label for="estado" class="control-label" >Mostrar:</label>\n\
   <select ng-param-directive  class="input-xs form-control w-xs inline v-middle" ng-model="pageCount" ng-options ="count for count in params.settings().counts" ng-change="params.count(pageCount)"\n\
    />\n\
</div>\n\
    <div   class="{{pages.length>0?\'col-sm-3\':\'col-sm-12\'}} text-center centered">\n\
     <small class="text-muted inline m-t-sm m-b-sm">{{((params.page()-1)*params.count())+1}} - {{(params.page()*params.count())>params.settings().total?params.settings().total:params.page()*params.count()}} de {{params.settings().total}}</small>\n\
    </div>\n\
        <div class="col-sm-5 text-right text-center-xs">\n\
        <ul class="pagination pagination-sm m-t-none m-b-none">\n\
           <li ng-class="{\'disabled\': !page.active}" ng-repeat="page in pages" ng-switch="page.type"> \n\
                                           <a ng-switch-when="prev" ng-click="params.page(page.number)" ng-init="ss=page.number" href="">&laquo;</a> \n\
                                           <a ng-switch-when="first" ng-click="params.page(page.number)" ng-init="ss=page.number" href="">{{page.number}}</a> \n\
                                           <a ng-switch-when="page" ng-click="params.page(page.number)" ng-init="ss=page.number" href="">{{page.number}}</a> \n\
                                           <a ng-switch-when="more" ng-click="params.page(page.number)" ng-init="ss=page.number" href="">&#8230;</a> \n\
                                           <a ng-switch-when="last" ng-click="params.page(page.number)" ng-init="ss=page.number"href="">{{page.number}}</a> \n\
                                           <a ng-switch-when="next" ng-click="params.page(page.number)" ng-init="ss=page.number" href="">&raquo;</a> \n\
              </li>\n\
          </ul>\n\
        </div>\n\
      </div>\n\
